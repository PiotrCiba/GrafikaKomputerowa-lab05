...
public class Lab5 extends GLJPanel implements GLEventListener, KeyListener{
	...
	private void draw(GL2 gl2) {
		...
		gl2.glRotatef((float)rotateZ, 0.0f, 0.0f, 1.0f);   // Apply rotations to complete object.
		gl2.glRotatef((float)rotateY, 0.0f, 1.0f, 0.0f);
		gl2.glRotatef((float)rotateX, 1.0f, 0.0f, 0.0f);
	...
	public void Pyramid(GL2 gl2) {
		double r = 3.0d;
		int n = 15;
		float[] cprim = new float[] {0.0f,1.0f,0.0f};
		float[] csec = new float[] {0.0f,0.7f,0.0f};
		float height = 8.0f;
		
		//rysowanie ścian piramidy
		gl2.glBegin(6);
		gl2.glVertex3f(0.0f, height*0.5f, 0.0f);
		gl2.glVertex3f(0.0f, -height*0.5f, 0.0f);
		
		int i;
		for(i = 0; i < n; ++i) {
			gl2.glVertex3f((float) (Math.cos((float) (i * 2) * Math.PI / (float) n) * r), -height*0.5f, (float) (Math.sin((float) (i * 2) * Math.PI / (float) n) * r));
			gl2.glColor3fv(cprim, 0);
			gl2.glVertex3f((float) (Math.cos((float) ((i + 1) * 2) * Math.PI / (float) n) * r), -height*0.5f, (float) (Math.sin((float) ((i + 1) * 2) * Math.PI / (float) n) * r));
			gl2.glColor3fv(csec, 0);
		}
	...
	public void Corkscrew(GL2 gl2) {
		gl2.glEnable(2832); // jakosc renderowania
		gl2.glColor3f(0.0f, 1.0f, 0.0f); // kolor
		float n = 16.0f; // ilosc punktów w pierscieniu
		float r = 1.5f; // szerokosc pierscieni (promień)
		float start = -5.0f; // punkt startu spirali
		float pointSize = 5.0f; // rozmiar punktow tworzacych spirale
		int nSpirals = 16; // liczba pierscieni

		for (int j = 0; j < nSpirals; ++j) {
			for (int i = 1; (float) i <= n; ++i) {
				float x = (float) ((float) r * Math.sin(Math.PI / (float) (n / 2.0f) * (float) i));
				float y = (float) ((float) r * Math.cos(Math.PI / (float) (n / 2.0f) * (float) i));
				gl2.glPointSize(pointSize);
				gl2.glBegin(0);
				gl2.glVertex3f(x, start, y);				//narysownanie punktu
				gl2.glEnd();
				start = ((float) start + 0.05f);			//przesunięcie kolejnego pkt o stałą 0.05 w osi spirali
				pointSize = ((float) pointSize + 0.05f);	//powiększenie kolejnego pkt o stałą 0.05
			}
		}
	}
